...
const processTable = (tableHTML: string) => {
  ...
  for (const element of document.querySelectorAll('tr').values()) {
    if (element.className === 'header') {
      ...
      if (licenseQuery && licenseHref) license = { type: licenseQuery, url: licenseHref };
    }

    const copyrightString = element.querySelector('.copyright')?.textContent?.trim().split(' ');
    if (id && license && copyrightString) {
      const emailArr = copyrightString.filter(string => EMAIL_REGEX.test(string));
      const websiteArr = copyrightString.filter(string => URL_REGEX.test(string));
      const email = emailArr.length > 0 ? emailArr.join(' ').replaceAll(BRACKETS_REGEX, '').replaceAll(',', '').trim().replaceAll(' ', ', ') : undefined;
      const website = websiteArr.length > 0 ? websiteArr.join(' ').replaceAll(BRACKETS_REGEX, '').replaceAll(',', '').replaceAll(' ', ', ') : undefined;
      const copyright = copyrightString.filter(string => !EMAIL_REGEX.test(string) && !URL_REGEX.test(string)).join(' ').replaceAll(BRACKETS_REGEX, '').replaceAll(DOUBLE_SPACE_REGEX, ' ').split(':')[1]?.trim();
