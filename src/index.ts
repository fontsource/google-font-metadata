...
const APIDirect = JSON.parse(
  fs.readFileSync(join(dirname(fileURLToPath(import.meta.url)), "../data/api-response.json")).toString()
) as APIResponse[];
...
const APIv1 = JSON.parse(
  fs.readFileSync(join(dirname(fileURLToPath(import.meta.url)), "../data/google-fonts-v1.json")).toString()
) as FontObjectV1;
 ...
 */
const APIv2 = JSON.parse(fs.readFileSync(join(dirname(fileURLToPath(import.meta.url)), "../data/google-fonts-v2.json")).toString()
) as FontObjectV2;
 ...
 */
const APIVariableDirect = JSON.parse(fs.readFileSync(join(dirname(fileURLToPath(import.meta.url)), "../data/variable-response.json")).toString()
 ...
 * Alternatively, the slower `npx gfm parse --variable` command can be used.
 */
const APIVariable = JSON.parse(fs.readFileSync(join(dirname(fileURLToPath(import.meta.url)), "../data/variable.json")).toString()
) as FontObjectVariable;

export { APIDirect, APIv1, APIv2, APIVariable, APIVariableDirect };

export type { APIResponse, FontObject, FontObjectV1, FontObjectV2, FontObjectVariable };
