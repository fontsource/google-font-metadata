...
import { fontObjectV1Schema, fontObjectV2Schema, fontObjectVariableSchema } from "./schema";

type Version = "v1" | "v2" | "variable";

const validate = (version: Version, data: FontObject) => {
    consola.info(`Validating metadata... ${colors.bold(colors.yellow(`[API ${version.toUpperCase()}]`))}`);
    switch (version) {
        case "v1": {
        ...
        case "variable": {
            ...
            if (!valid.success) {
                throw new Error(`Invalid parse for variable! Try running ${colors.yellow("npx gfm generate --variable && npx gfm parse --variable")}. If the problem still persists, Google may have tweaked their API. Please make an issue on google-font-metadata.\n${valid.error}`);
            }
...
const validateCLI = (version: Version) => {
    ...
    switch (version) {
        ...
        default: throw new Error("Invalid validation version.");
    }
